  ...
  export default {
    ...
    methods: {
      ...
      wxReady() {
        ...
        let redirect = 'http://' + location.host + '/weixin/b/' + this.good.id;
        let uid = this.$store.getters.getUserInfo.userId;
        if (uid) {
          redirect += '?userId=' + uid;
        }
        ...
        if (this.good.icon) {
          icon = api.CONFIG.psCtx + this.good.icon + '?w=423&h=423';
        }
        let vm = this;
        let shareData = {
          title: `[${api.CONFIG.NICKNAME || '一虎一席'}.团购${this.good.groupPrice}元] ` + reduceGoodsName(this.good.name),
          desc: '团购：¥' + this.good.groupPrice + `.「${api.CONFIG.APPNAME || '一虎一席茶生活美学商城'}」精品.【一站式优品商城，品味脱凡】`,
          link: redirect,
          imgUrl: icon,
          success: function () {
