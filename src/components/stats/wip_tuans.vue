<template>
  <div>
    <fixedheader title="正在拼团的订单"></fixedheader>
    <div class="stats">
        ...
        <div class="stats-container" ref="statList" v-show="orders.length">
          <mu-flexbox wrap="wrap" justify="space-around" :gutter="0" class="detail_list">
            <ul class="newitem head">
              <li class="col-2">团长</li>
              <li class="col-2">拼团商品</li>
              <li class="col-2">当前进度</li>
            ...
            <mu-flexbox-item basis="100%" class="newitem border-1px" v-for="(item, idx) in orders" :key="idx">
              <div class="col-2" @click.stop.prevent="showDetail(item)"><img :src="getTuanOwner(item).userIcon" class="thumbnail" />{{getFriendlyUsername(getTuanOwner(item).userName)}}</div>
              <div class="col-2" @click.stop.prevent="showDetail(item)">{{item.name}}</div>
              <div class="col-2" @click.stop.prevent="showDetail(item)">已拼{{item.joinCount}}人 • {{item.limitCount}}人</div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      showDetail(item) {
        this.$router.push({name: 'sharetuan', params: {id: item.fieldId}, query: {tuanId: item.id}});
      },
      getTuanOwner(item) {
        let tuanInfo = item.infos || [];
        return tuanInfo.find(user => user.owner) || {};
      },
      show() {
